...
class PaschaClient {
   ...
   * @param {Object} options - { ip, userAgent, headers, geo, behaviorData, canvasFingerprint }
  ...
  async detectVisitor(options = {}) {
    ...
    try {
      ...
      console.log('[PASCHA_CLIENT] Enviando para backend:', {
        ip,
      ...
      console.log('[PASCHA_CLIENT] Base URL:', this.baseUrl);
      console.log('[PASCHA_CLIENT] API Key present:', !!this.apiKey);
      ...
      if (!response.ok) {
        ...
        console.error('[PASCHA_CLIENT] API error status:', response.status, 'Body:', errorText);
        throw new Error(`API error: ${response.status} - ${errorText}`);
      }

      const result = await response.json();
      console.log('[PASCHA_CLIENT] Raw response from PASCHA:', JSON.stringify(result, null, 2));
      ...
      console.error('[PASCHA] ❌ Erro ao detectar visitante:', {
        message: errorMessage,
        name: error.name,
        url: `${this.baseUrl}/api/detect-visitor`,
